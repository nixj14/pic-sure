...
public class HSAPIResourceRS implements IResourceRS
{
	...
	public static String targetURL = System.getenv("HSAPI_TARGET_URL");
	...
	public ResourceInfo info(QueryRequest queryRequest) {
		...
		HSAPIresourceInfo.setName("HSAPI Resource : " + targetURL);
	...
	private String buildPath (QueryRequest request){
		...
		if (node.has("page")){
			...
			return composeURL(targetURL, path, query);
		}

		return composeURL(targetURL, path);
	}
